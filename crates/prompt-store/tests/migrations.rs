...
#[test]
fn upgrades_a_v4_database_with_prompt_usage_tracking() {
    let directory = tempdir().unwrap();
    ...
    assert_eq!(database.schema_version().unwrap(), LATEST_SCHEMA_VERSION);
    let upgraded = Connection::open(&path).unwrap();
    let column_exists: i64 = upgraded
        .query_row(
    ...
    let first_index_column: String = upgraded
        .query_row(
            "SELECT name FROM pragma_index_info('idx_prompts_last_used') WHERE seqno = 0",
            [],
            |row| row.get(0),
        )
        .unwrap();
    assert_eq!(first_index_column, "last_used_at");
}
